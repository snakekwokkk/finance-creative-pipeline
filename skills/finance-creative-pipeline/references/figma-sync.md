...
- Sync only manifest directions whose status is `ready` and whose `preview.png`, `layers.json`, and `layers/decomposition-report.json` all exist and validate. The decomposition report may be `ready` or `partial` when at least one accepted complex asset exists. Ignore retained `failures` except as audit evidence.
