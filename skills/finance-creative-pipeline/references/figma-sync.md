...
4. Call `upload_assets` for `preview.png` and only separate PNG files whose `extractionMode` is `native-source-pixel-asset`, whose `asset.engine` is `native-source-pixel-matting`, whose `asset.sourcePixelExact` is `true`, and whose `asset.status` is `accepted`. Place each accepted asset inside `assetPlacement.box` (or `assetBboxPx` for older reports) with aspect ratio preserved, `contain` sizing, and center alignment; never stretch it to fill a box whose aspect ratio differs from `assetIntrinsicPx`. Never upload stale ChatGPT-redrawn assets or synthesize a rectangular crop for `transparent-asset-rejected` layers.
