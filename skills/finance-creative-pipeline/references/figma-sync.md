...
Use this reference as soon as the local runtime emits `direction_ready`. The event starts a five-minute cooldown from decomposition completion. Complete this direction's Figma reconstruction and QA while that timer runs; the producer will not generate the next direction until both the cooldown and current-direction QA are complete.
...
- `Editable`: frame containing a locked hidden `Visual Base` and visible `Editable Elements` made from accepted ChatGPT batch-generated transparent PNGs, native OCR text, Remix Icon vectors, buttons, backplates, and semantic metadata. Never use the flattened preview as the visible editable-side output. For popup directions the outer canvas is transparent and contains only the popup body; Banner and float directions may contain native backgrounds.
...
4. Call `upload_assets` for `preview.png` and every accepted PNG whose `extractionMode` is `chatgpt-generated-transparent-asset`, with `asset.engine: chatgpt-batch-transparent` and `sourcePixelExact: false`. Place each accepted asset inside `assetPlacement.box` with aspect ratio preserved, `contain` sizing, and center alignment; never stretch it to fill a box whose aspect ratio differs from `assetIntrinsicPx`. Never upload `transparent-asset-rejected` layers. A `partial` report is eligible for handoff when it contains at least one accepted image.
5. Build every word, number, amount, unit, CTA label, card, button, rectangle, background, simple gradient, divider, border, and simple shadow natively from `layers.json`; a legacy `spec.json` is optional supplementary context only. Resolve ordinary `kind: "icon"` layers through the Remix Icon workflow below instead of drawing paths manually. Do not treat a red-envelope, envelope, card, panel, or button backplate as a raster merely because it is prominent. For popup directions, do not create a canvas fill or reconstruct anything behind the popup card. Ignore legacy popup layers whose roles or IDs describe App/page backgrounds, search bars, navigation, bottom tabs, feeds, page cards, or other environmental UI. Banner and float directions may build their native background fills from the background layer's geometry and style, using a legacy `spec.json.palette` only as a fallback. Load fonts before every text mutation. Keep `Visual Base` locked and hidden. Rejected or missing assets remain absent; never reveal the flattened preview to conceal an incomplete editable reconstruction.
