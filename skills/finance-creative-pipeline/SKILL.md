...
3. Run `node scripts/run.mjs` for the normal 20-reference/10-direction workflow, add `--test` for the 3-reference/1-popup trial, or use `--types popup,banner,float` for an isolated three-type validation run with two references and one generated direction per requested type.
